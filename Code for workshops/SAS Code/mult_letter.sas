...
/*                                                                                           */
/* In addition to the printed output appearing in the output window, results are also stored */
/* in a SAS dataset means_and_letters                                                        */
/*                                                                                           */
/*                                                                                           */
...
/* 6 February 2012 addes descending option                                                   */
/* 8 February 2012 added more convenient mode of slicing                                     */
/* 9 June 2012 store output in SAS dataset (suggested by Edzard van Santen                   */
...
if _N_>1;
run;
* Capturing the highest Column used as a macro variable;
proc sql; 
	select name into :maxCol from dictionary.columns where libname = 'WORK' 
			and memname='LETTERS0' having varnum = max(varnum);
quit;

data means_and_letters;
	merge lsmeans letters0;
	Letter = CATT(of Col1 - &maxCol);*concatening columns with letters;
	drop _type_ _freq_ tvalue probt df Col1-&maxCol;
run;
...
if check={0 0 0} then print trt label lsmean letters;
if check={1 0 0} then print trt by level label lsmean letters;
if check={1 1 0} then print trt by level by2 level2 Label lsmean letters;
if check={1 1 1} then print trt by level by2 level2 by3 level3 Label lsmean letters;

end;  /*need <=26 letters*/

/*output it all into a SAS dataset (suggestion by Ezard van Santen, 9 June 2012*/

create letters from letters;
append from letters;
...
title 'Standard errors of a difference (SED) and Least significant differences (LSD)';
proc means data=lsd min mean max;
var sed lsd;
output out=lsd_sed mean=sed_mean lsd_mean min=sed_min lsd_min max=sed_max lsd_max;
run;
...
data letters;
merge lsd_sed letters;
run;

data letters0; set letters0 letters;
